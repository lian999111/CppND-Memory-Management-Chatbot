...
ChatBot::ChatBot(const ChatBot &rhs)
    : _currentNode(rhs._currentNode), _rootNode(rhs._rootNode),
      _chatLogic(rhs._chatLogic), _image(nullptr) {
  std::cout << "ChatBot copy ctor called." << std::endl;

  // Deep copy owned _image
  if (rhs._image != nullptr) {
    _image = new wxBitmap();
    *_image = *rhs._image;
  }
}
...
ChatBot &ChatBot::operator=(const ChatBot &rhs) {
  ...
  if (this != &rhs) {
    ...
    // Owned
    // Release _image if allocated already
    if (_image != nullptr) {
      delete _image;
      _image = nullptr;
    }
...
ChatBot &ChatBot::operator=(ChatBot &&rhs) {
  ...
  // No need to check self-move
  // Ref: http://ericniebler.com/2017/03/31/post-conditions-on-self-move/

  // Release _image if allocated
  if (_image != nullptr) {
    delete _image;
    _image = nullptr;
  }
