...
ChatLogic::ChatLogic()
{
    ...
    // No need to create instance of chatbot
...
ChatLogic::~ChatLogic()
{
    ...
    // No need to delete chatbot instance since it's not owned by ChatLogic
    // delete _chatBot;
...
void ChatLogic::LoadAnswerGraphFromFile(std::string filename)
{
            ...
            if (type != tokens.end())
            {
                ...
                if (idToken != tokens.end())
                {
                    ...
                    if (type->second == "NODE")
                    {
                        ...
                        // The lambda function takes a const reference to unique_ptr<GraphNode> so copying is avoided, which violates the policy of unique_ptr 
                    ...
                    if (type->second == "EDGE")
                    {
                        ...
                        if (parentToken != tokens.end() && childToken != tokens.end())
                        {
                            ...
                            // ChatLogic doesn't own ptrs to edges anymore after adaptation
                            ...
                            (*childNode)->AddEdgeToParentNode(edge.get());      // pass the raw ptr to the edge to the child node
                            (*parentNode)->AddEdgeToChildNode(std::move(edge)); // move the unique_ptr to the edge to the parent node 
                        }
    ...
    // Create a ChatBot on stack
    ChatBot chatBot = ChatBot("../images/chatbot.png");
    // Set chatbot's chatlogic handle
    chatBot.SetChatLogicHandle(this);

    // Add chatbot to graph root node
    chatBot.SetRootNode(rootNode);
    _chatBot = &chatBot;    // ChatLogic still has a member raw ptr to ChatBot for communication

    // Move chatbot instance to root node
